...
def create_convnext_diagram(output_dir="viz/diagrams"):
    ...
    for name, x, y, color, width in components:
        rect = patches.Rectangle(
            (x, y - 0.7), width, 1.4, facecolor=color, edgecolor="black", linewidth=2
        )
        ax.add_patch(rect)
    ...
    ax.axis("off")
    ax.set_title(
        "ConvNeXt-V2 Architecture with Global Response Normalization",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
...
def create_vit_diagram(output_dir="viz/diagrams"):
    ...
    for name, x, y, color, width in components:
        rect = patches.Rectangle(
            (x, y - 0.8), width, 1.6, facecolor=color, edgecolor="black", linewidth=2
        )
        ax.add_patch(rect)
        ax.text(x + width / 2, y, name, ha="center", va="center", fontsize=9, fontweight="bold")
        ...
        if x < 16:
            ax.arrow(
                x + width, y, 0.4, 0, head_width=0.15, head_length=0.15, fc="black", ec="black"
            )
...
def create_efficientnet_diagram(output_dir="viz/diagrams"):
    ...
    for name, x, y, color, width in components:
        ...
        if x < 15:
            ax.arrow(
                x + width, y, 0.15, 0, head_width=0.1, head_length=0.1, fc="black", ec="black"
            )
    ...
    ax.axis("off")
    ax.set_title(
        "EfficientNet Architecture with Mobile Optimization",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
...
def create_multihead_diagram(output_dir="viz/diagrams"):
    ...
    for name, x, y, color, width in backbone:
        rect = patches.Rectangle(
            (x, y - 0.5), width, 1, facecolor=color, edgecolor="black", linewidth=2
        )
        ax.add_patch(rect)
