...
\begin{slide}[\slideopts,toc={Snapshot}]{Project Snapshot — Lightning Hydra Template Extended for Audio}
  ...
  \begin{itemize}
    ...
    \mpitem Drops MNIST/CIFAR baselines, adds audio-specific tooling and interactive evaluation
  \end{itemize}
\end{slide}
...
\begin{slide}[\slideopts,toc={Spectra}]{Spectral-First Audio Representations}
  \begin{itemize}
    \mpitem End-to-end pipelines are feasible but demand \emph{massive} data, compute, and wall-clock time
    \mpitem NSMT adopts inductive priors: the ear is effectively a hardware spectrum analyzer
    \mpitem Encode alternative time-frequency representations in stacked spectrogram ``channels'' (time-frequency resolutions, modulation spectra)
    \mpitem Conditioning (``aux'') inputs bring in measured features (F0, decay-time, pre-emphasis)
    \mpitem Conditioning inputs add context
  \end{itemize}
  \vspace{-0.5em}
  \maybepause
    \centerline{\textit{Those who don't know signal processing are doomed to reinvent it}}
...
\begin{wideslide}[\slideopts,toc={Context}]{Anchoring in Human Audio Research}
  ...
  \begin{itemize}
    ...
    \mpitem Shared convolutional front-end generalized across speech and music; input priors mattered
    \mpitem Cochleagram input was required to predict cortical responses
    \mpitem NSMT adopts similar spectral priors, adding multi-resolution stacks and conditioning heads
  \end{itemize}
}
\end{wideslide}
...
\begin{slide}[\slideopts,toc={Layout}]{Repository Layout \& Key Modules}
  ...
  \begin{itemize}
    ...
    \mpitem PyTorch Lightning modules in \texttt{src/models/} for models and \texttt{src/data/} for data loading
    \mpitem Tests in \texttt{tests/} (fast by default, slow marked); assets: \texttt{data/}, \texttt{logs/}, \texttt{viz/}, \texttt{docs/}
  \end{itemize}
\end{slide}
...
\begin{slide}[\slideopts,toc={VIMH}]{VIMH Dataset Format — Self-Describing, Multihead}
  ...
  \begin{itemize}
    ...
    \mpitem Metadata file \texttt{vimh\_dataset\_info.json} declares label ids and 0--255 ranges
    \mpitem Supports 0--255 quantized continuous parameters and (0--255) parameter/category names
  \end{itemize}
  \vspace{0.25em}
  \begin{center}
    \begin{tikzpicture}[x=1mm,y=1mm, line join=round, line cap=round]
...
\begin{slide}[\slideopts,toc={VIMH Display 1}]{VIMH Display Utility \texttt{display\_vimh.py}, Tab 1}
  \begin{center}
    \includegraphics[width=\linewidth]{eps/display_vimh_fig1.eps}
  \end{center}
\end{slide}
\begin{slide}[\slideopts,toc={VIMH Display 1b}]{VIMH Display Utility \texttt{display\_vimh.py}, Tab 1, Higher Wah}
  \begin{center}
    \includegraphics[width=\linewidth]{eps/display_vimh_fig1b.eps}
  \end{center}
\end{slide}
\begin{slide}[\slideopts,toc={VIMH Display 2}]{VIMH Display Utility \texttt{display\_vimh.py}, Tab 2}
  ...
  \end{center}
\end{slide}
\begin{slide}[\slideopts,toc={VIMH Display 2b}]{VIMH Display Utility \texttt{display\_vimh.py}, Tab 2, Higher Wah}
  \begin{center}
    \includegraphics[width=0.6\linewidth]{eps/display_vimh_fig2b.eps}
  \end{center}
\end{slide}
\begin{slide}[\slideopts,toc={VIMH Display 3}]{VIMH Display Utility \texttt{display\_vimh.py}, Tab 3}
  \begin{center}
    \includegraphics[width=0.6\linewidth]{eps/display_vimh_fig3.eps}
  \end{center}
\end{slide}
\begin{slide}[\slideopts,toc={VIMH Display 3b}]{VIMH Display Utility \texttt{display\_vimh.py}, Tab 3, Higher Wah}
  \begin{center}
...
\begin{slide}[\slideopts,toc={Models}]{Model Portfolio \& Auto-Configuration}
  \vspace{-0.75em}
  \begin{itemize}
    \mpitem CNNs: \texttt{cnn\_micro}, \texttt{cnn\_tiny}, \texttt{cnn\_medium} with ordinal/regression/aux heads
    \mpitem Lightweight ViTs: \texttt{vit\_micro} and \texttt{vit\_tiny} with spectrogram-aware patches
    \mpitem Auto-wiring pipeline:
    \begin{itemize}
      \mpitem DataModule ingests \texttt{vimh\_dataset\_info.json} to expose shapes + label semantics
      \mpitem \texttt{VIMHLitModule} instantiates heads, losses, and metrics per label type
      \mpitem Hydra selects trainer (CPU/GPU/MPS) and callbacks (checkpoints, early stop, diagrams)
    \end{itemize}
    \mpitem Result: one dataset $\to$ consistent model wiring and reproducible runs
  \end{itemize}
\end{slide}
...
\begin{slide}[\slideopts,toc={Losses}]{Losses, Metrics, and Diagnostics}
  \vspace{-0.75em}
  \begin{itemize}
    \mpitem Quantized parameters (0--255) need distance-aware objectives:
    \begin{itemize}
...
\begin{slide}[\slideopts,toc={Workflow}]{Training \& Experiment Workflow}
  ...
  \begin{itemize}
    ...
    \mpitem \textbf{Training:} \texttt{make tq} (smoke test) \textbar \texttt{make tr} (default) \textbar \texttt{python src/train.py experiment=emb trainer=gpu}
    \mpitem \textbf{Overrides:} \texttt{model=cnn\_medium\_ordinal} \textbar \texttt{data=vimh} \textbar \texttt{trainer.max\_epochs=50}
...
\begin{slide}[\slideopts,toc={Evaluation}]{Evaluation \& Audio Reconstruction Tools}
  ...
  \begin{itemize}
    ...
    \mpitem Metrics: SNR, correlation, RMSE, per-parameter errors with exportable tables
    \mpitem Batch comparisons push snapshots and notes back to \texttt{logs/}
  \end{itemize}
  \vspace{0.25em}
  \begin{center}
...
\begin{slide}[\slideopts,toc={Experiments}]{Moog VCF Case Studies — Basic, Envelope, Resonance}
  ...
  \begin{itemize}
    ...
    \mpitem \textbf{Runs:} \texttt{emb}, \texttt{eme}, \texttt{emr} with CNN/ViT variants (\texttt{emvit*})
    \mpitem Study generalization across parameter spaces, envelopes, and resonance settings
    \mpitem Findings: ordinal losses stabilize extrapolation; ViTs shine on envelopes, CNNs win on jittered resonance
  \end{itemize}
  \vspace{0.25em}
  \begin{center}
...
\begin{slide}[\slideopts,toc={Practices}]{Engineering Practices, Reproducibility, Next Steps}
  \vspace{-0.75em}
  \begin{itemize}
    \mpitem Hydra configs capture overrides; each run logs command + git SHA in \texttt{logs/train/runs/}
    \mpitem Preflight scripts audit label diversity and dataset integrity before training
...
\begin{slide}[\slideopts,toc={Losses}]{[ORIG] Distance-Aware Losses for Quantized Parameters}
  \vspace{-0.75em}
  \begin{itemize}
...
\begin{slide}[\slideopts,toc={VIMH Loss Analysis}]{VIMH Loss Analysis}
  \begin{center}
    \includegraphics[width=\linewidth]{eps/vimh_loss_analysis.eps}
  \end{center}
\end{slide}
...
%% DOWN IN THE WEEDS AND NEEDING TOO MUCH WORK - ALL ``64K'' NEED TO BECOME ``Medium'' OR SUCH
%% \begin{slide}[\slideopts,toc={Models}]{Model Families — Small, Accurate, and Fast}
%%   \vspace{-0.75em}
%%   \begin{itemize}
%%     \mpitem \textbf{CNN variants:} \texttt{cnn\_micro} ($\approx 2$K), \texttt{cnn\_tiny} ($\approx 8$K), \texttt{cnn\_medium} ($\approx 1400$K)
%%     \vspace{-0.25em}
%%     \begin{itemize}
%%       \mpitem Multihead heads auto-wired from VIMH metadata
%%       \mpitem Options: \texttt{cnn\_medium\_ordinal}, \texttt{cnn\_medium\_regression}, \texttt{cnn\_medium\_auxiliary}
%%     \end{itemize}
...
%% TOO CODE BASED - COVERED ELSEWHERE
...
\begin{slide}[\slideopts,toc={Workflow}]{Hands-On Workflow --- Datasets, Training, Diagrams}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \mpitem \texttt{python src/train.py model=cnn\_medium\_ordinal trainer=mps}
    \end{itemize}
