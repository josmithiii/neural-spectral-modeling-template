...
def print_dict_structure(
    data: Dict[str, Any], prefix: str = "", max_depth: int = 3, current_depth: int = 0
) -> None:
...
def inspect_checkpoint(ckpt_path: str) -> None:
    ...
    try:
        ...
        if "hyper_parameters" in checkpoint:
            ...
            synth_keys = [
                k for k in hyper_params.keys() if "synth" in k.lower() or "param" in k.lower()
            ]
            if synth_keys:
        ...
        if hyper_params:
            data_keys = [
                k for k in hyper_params.keys() if "data" in k.lower() or "dataset" in k.lower()
            ]
