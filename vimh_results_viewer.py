...
class VIMHResultsViewer:
    ...
    def _load_dataset(self):
        ...
        self.datamodule = VIMHDataModule(
            data_dir=self.dataset_path, batch_size=1, num_workers=0  # Single sample for viewing
        )
            ...
            if config_path.exists():
                try:
                    ...
                    if "model" in config_data and "net" in config_data["model"]:
                        ...
                        print(
                            f"  • Conv channels: {cnn_config['conv1_channels']}, {cnn_config['conv2_channels']}"
                        )
    ...
    def _load_model(self):
        ...
        try:
            ...
            if dataset_heads != model_heads:
                print(
                    f"WARNING: Dataset heads {dataset_heads} don't match model heads {model_heads}"
                )
    ...
    def _setup_widgets(self):
        ...
        self.sample_slider = Slider(
            ax_slider, "Sample", 0, max(0, self.max_samples - 1), valinit=0, valfmt="%d"
        )
    ...
    def _update_spectrogram(self, image: torch.Tensor):
        ...
        # Display spectrogram
        im = self.ax_spec.imshow(img_data, aspect="auto", origin="lower", cmap="viridis")
    ...
    def _update_accuracy(self, true_labels: Dict[str, int], predictions: Dict[str, Dict]):
        ...
        for i, (acc, conf) in enumerate(zip(accuracies, confidences)):
            ...
            self.ax_accuracy.text(conf + 0.02, i, f"{symbol} {conf:.2f}", va="center", fontsize=10)
    ...
    def _update_status(self, true_labels: Dict[str, int], predictions: Dict[str, Dict]):
        ...
        avg_confidence = (
            np.mean([predictions[name]["confidence"] for name in true_labels.keys()])
            if total > 0
            else 0
        )
...
def main():
    ...
    try:
        viewer = VIMHResultsViewer(
            dataset_path=args.dataset, checkpoint_path=args.checkpoint, logs_dir=args.logs_dir
        )
