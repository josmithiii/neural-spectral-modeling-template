...
\begin{tikzpicture}[scale=1.0]
  ...
  \def\conv1height{3.2}
  \begin{scope}[shift={(1*\xshift,0.4)}]
  ...
  \def\conv2height{2.4}
  \begin{scope}[shift={(2*\xshift,0.8)}]
  ...
  % AdaptiveAvgPool
  \def\poolheight{1.6}
  \begin{scope}[shift={(3*\xshift,1.2)}]
  ...
  \def\fcheight{2.0}
  \begin{scope}[shift={(4*\xshift,1.0)}]
  ...
  \def\outheight{1.2}
  \begin{scope}[shift={(5*\xshift,1.4)}]
  ...
  % Add arrows between layers - horizontal along center line
  \foreach \i in {0,1,2,3,4} {
    \pgfmathtruncatemacro{\nexti}{\i+1}
    \draw[->, thick, gray!70]
      (\i*\xshift+\layerwidth+\layerdepth*0.7+0.1, 2.0) --
      (\nexti*\xshift-0.1, 2.0);
  }
