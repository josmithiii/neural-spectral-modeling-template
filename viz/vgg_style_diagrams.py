...
class VGGStyleDiagram:
    ...
    def add_text_label(
        self, ax, x: float, y: float, text: str, fontsize: int = 10, fontweight: str = "normal"
    ) -> None:
    ...
    def create_multihead_outputs(
        ...
        for i in range(heads_to_show):
            ...
            self.create_3d_block(
                ax, start_x, y_pos - head_height / 2, 1.2, head_height, 0.2, self.colors["output"]
            )
    ...
    def create_cnn_diagram(
        self, model_name: str, architecture: Dict[str, Any], save_path: str
    ) -> None:
        ...
        ax.text(
            10,
            9.5,
            f"{model_name.upper()} Architecture",
            fontsize=16,
            fontweight="bold",
            ha="center",
        )
        ax.text(
            10,
            9,
            f"Parameters: {architecture['total_params']:,}",
            fontsize=12,
            ha="center",
            style="italic",
        )
        ...
        self.create_3d_block(ax, current_x, block_y - 1, 1.2, 2, 0.3, self.colors["input"])
        ...
        for i, conv_block in enumerate(architecture["conv_blocks"]):
            ...
            self.add_text_label(ax, current_x + block_width / 2, block_y + 0.5, f"Conv{i+1}")
            self.add_text_label(
                ax, current_x + block_width / 2, block_y + 0.1, f"{channels} filters"
            )
            self.add_text_label(
                ax,
            ...
            if params > 0:
                self.add_text_label(
                    ax, current_x + block_width / 2, block_y - 0.7, f"{params:,} params"
                )
            ...
            if conv_block.get("pool", False):
                ...
                pool_width = 0.6
                self.create_3d_block(
                    ax, current_x, block_y - 0.5, pool_width, 1, 0.2, self.colors["pool"]
                )
        ...
        feature_width = 1.0
        self.create_3d_block(
            ax, current_x, block_y - 1, feature_width, 2, 0.2, self.colors["pool"]
        )
        ...
        self.add_text_label(
            ax, current_x + feature_width / 2, block_y - 1.3, f"{flattened_features} features"
        )
        ...
        if show_auxiliary:
            ...
            self.create_3d_block(
                ax,
                aux_start_x - aux_input_width / 2,
                block_y + aux_y_offset - 0.4,
                aux_input_width,
                0.8,
                0.15,
                "#9013FE",
            ...
            if isinstance(aux_input_size, str):
                ...
                size_label = f"{aux_input_size}D"
            self.add_text_label(
                ax, aux_start_x, block_y + aux_y_offset - 0.6, size_label, fontsize=7
            )

            # Auxiliary MLP
            aux_mlp_x = aux_start_x + 1.5
            self.create_3d_block(
                ax, aux_mlp_x - 0.6, block_y + aux_y_offset - 0.4, 1.2, 0.8, 0.15, "#7ED321"
            )  # Green like FC
            self.add_text_label(ax, aux_mlp_x, block_y + aux_y_offset, "Aux MLP", fontsize=8)
            self.add_text_label(
                ax, aux_mlp_x, block_y + aux_y_offset - 0.6, f"→{aux_hidden_size}D", fontsize=7
            )
        ...
        if is_multihead:
            ...
            if shared_fc:
                ...
                aux_input_size = architecture.get("auxiliary_input_size", 0)
                aux_hidden_size = architecture.get("auxiliary_hidden_size", 32)
                show_auxiliary = has_auxiliary and (
                    isinstance(aux_input_size, str) or aux_input_size > 0
                )

                if show_auxiliary:
                    self.add_text_label(
                        ax, current_x + fc_width / 2, block_y + 0.7, "Shared\nFeatures"
                    )
                    self.add_text_label(
                        ax,
                        current_x + fc_width / 2,
                        block_y + 0.2,
                        f"{shared_fc['out_features']} units",
                    )
                    self.add_text_label(
                        ax, current_x + fc_width / 2, block_y - 0.2, "(concat with aux)"
                    )
                    ...
                    self.add_text_label(
                        ax, (aux_mlp_x + current_x) / 2, concat_y - 0.3, "concat", fontsize=7
                    )
                else:
                    self.add_text_label(
                        ax, current_x + fc_width / 2, block_y + 0.5, "Shared\nFeatures"
                    )
                    self.add_text_label(
                        ax,
                        current_x + fc_width / 2,
            ...
            if heads_info:
                ...
                if num_heads >= 1:
                    if num_heads == 1:
                        ...
                        self.add_text_label(
                            ax, current_x + 0.6, lower_y - head_height / 2 - 0.4, label, fontsize=8
                        )

                        current_x += 1.2
                    else:
                        # Multiple heads - use original logic
                        self.create_multihead_outputs(
                            ax, current_x, block_y, num_heads, head_names, arrow_gap
                        )
            ...
            for i, fc_layer in enumerate(architecture["fc_layers"]):
                ...
                if fc_layer.get("activation"):
                    self.add_text_label(
                        ax, current_x + fc_width / 2, block_y - 0.7, fc_layer["activation"]
                    )
                ...
                if is_output:
                    ...
                    self.add_text_label(
                        ax, current_x + fc_width / 2, block_y - fc_height / 2 - 0.6, label
                    )
        ...
        # Save as EPS
        plt.tight_layout()
        plt.savefig(save_path, format="eps", dpi=300, bbox_inches="tight", pad_inches=0.2)
        plt.savefig(
            save_path.replace(".eps", ".png"),
            format="png",
            dpi=300,
            bbox_inches="tight",
            pad_inches=0.2,
    ...
    def create_vit_diagram(
        ...
        ax.text(
            10,
            9.5,
            f"{model_name.upper()} Vision Transformer",
            fontsize=16,
            fontweight="bold",
            ha="center",
        )
        ...
        self.create_3d_block(ax, current_x, block_y - 1, 1.5, 2, 0.3, self.colors["input"])
        ...
        self.add_text_label(
            ax, current_x + 0.75, block_y - 0.3, f"{patch_size}×{patch_size} patches"
        )
        ...
        for i in range(min(num_layers, 3)):  # Show max 3 blocks visually
            ...
            # Transformer block
            self.create_3d_block(ax, current_x, block_y - 1.2, 2, 2.4, 0.4, self.colors["conv"])
            ...
            if i == 1 and num_layers > 3:  # Show ellipsis
                ax.text(current_x - 0.5, block_y, "...", fontsize=20, ha="center", va="center")
                current_x += 1

        # Classification head
        self.add_arrow(ax, current_x - 1.5, current_x, block_y)
        self.create_3d_block(ax, current_x, block_y - 0.8, 1.5, 1.6, 0.3, self.colors["fc"])
    ...
    try:
        ...
        with initialize_config_dir(config_dir=config_dir, version_base=None):
            ...
            if architecture["model_type"] == "vit":
                ...
                is_multihead_style = (
                    has_heads_config and len(model.net.heads_config) >= 1
                ) or actual_multihead
                ...
                if has_heads_config and "ordinal" in config_name.lower():
                    ...
                    if hasattr(model.net, "classifier") or hasattr(model.net, "fc"):
                        classifier = getattr(
                            model.net, "classifier", getattr(model.net, "fc", None)
                        )
                        if classifier:
                            # Handle both single layer and sequential layers
                            if hasattr(classifier, "__iter__") and not hasattr(
                                ...
                                for layer in classifier:
                                    if hasattr(layer, "out_features"):
                                        activation = (
                                            "ReLU"
                                            if hasattr(layer, "relu") or "ReLU" in str(layer)
                                            else None
                                        )
                                        layer_params = (
                                            sum(p.numel() for p in layer.parameters())
                ...
                if not fc_layers:
                    ...
                    if "64k" in config_name:
                        fc_layers = [
                            ...
                            {"out_features": output_dim, "activation": None},
                        ]
                    elif "tiny" in config_name:
                        fc_layers = [
...
def main():
    ...
    parser.add_argument(
        "--output-dir", default="diagrams/vgg_style", help="Output directory for EPS diagrams"
    )
    parser.add_argument(
        "--models",
        nargs="+",
