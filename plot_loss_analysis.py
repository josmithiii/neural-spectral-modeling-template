...
def plot_loss_analysis(df: pd.DataFrame, save_path: str = None):
    ...
    ax1 = axes[0, 0]
    ax1.plot(
        train_data["epoch"], train_data["train/loss"], "b-", label="Training Loss", linewidth=2
    )
    ax1.plot(
        train_data["epoch"], train_data["val/loss"], "r-", label="Validation Loss", linewidth=2
    )
    ...
    ax1.annotate(
        f"Min Train Loss: {min_train_loss:.3f}",
    ...
    ax2.plot(
        train_data["epoch"],
        train_data["train/note_number_acc"] * 100,
        "g-",
        label="Note Number (Train)",
        linewidth=2,
    )
    ax2.plot(
        train_data["epoch"],
        train_data["train/note_velocity_acc"] * 100,
        "orange",
        label="Note Velocity (Train)",
        linewidth=2,
    )
    ax2.plot(
        train_data["epoch"],
        train_data["val/note_number_acc"] * 100,
        "g--",
        label="Note Number (Val)",
        linewidth=2,
    )
    ax2.plot(
        train_data["epoch"],
        train_data["val/note_velocity_acc"] * 100,
        "--",
        color="orange",
        label="Note Velocity (Val)",
        linewidth=2,
    )
    ...
    # Add random baseline
    ax2.axhline(
        y=100 / 256, color="red", linestyle=":", alpha=0.7, label="Random Baseline (0.39%)"
    )
    ...
    if "lr-Adam" in train_data.columns:
        ...
        lr_change_epoch = (
            train_data[train_data["lr-Adam"] == 0.0005].iloc[0]["epoch"]
            if len(train_data[train_data["lr-Adam"] == 0.0005]) > 0
            else None
        )
        if lr_change_epoch:
            ax3.annotate(
        ...
        ax3.plot(
            train_data["epoch"], loss_improvement, "purple", linewidth=2, marker="o", markersize=4
        )
        ax3.set_ylabel("Loss Improvement")
        ax3.set_title("Training Loss Improvement")
        ax3.annotate(
            "From manual analysis: LR 0.001→0.0005 at epoch ~12",
            xy=(0.5, 0.5),
    ...
    ax3.set_title(
        ...
        if "lr-Adam" in train_data.columns
        else "Training Loss Improvement"
    )
...
def analyze_loss_behavior(df: pd.DataFrame):
    ...
    print(
        f"  Training Loss Improvement: {train_data['train/loss'].iloc[0] - train_data['train/loss'].iloc[-1]:.4f}"
    )
    print(f"  Final Validation Loss: {train_data['val/loss'].iloc[-1]:.4f}")
    print(
        f"  Generalization Gap: {train_data['val/loss'].iloc[-1] - train_data['train/loss'].iloc[-1]:.4f}"
    )
...
def main():
    ...
    parser.add_argument(
        "--save_path", type=str, default="vimh_loss_analysis.png", help="Path to save the plot"
    )
