...
class AudioReconstructionEvaluator:
    ...
    def _normalize_parameters_for_plot(self, param_names: List[str], true_vals: List[float], pred_vals: List[float]) -> Tuple[List[float], List[float]]:
        """
    ...
    def _play_audio_multiple_methods(self, audio: np.ndarray, sample_idx: int, audio_type: str) -> None:
        """
    ...
    def plot_comparison(self, results: Dict[str, Any]) -> None:
        ...
        plt.subplot(3, 3, 1)
        plt.imshow(input_spec_display, aspect='auto', origin='lower', cmap='viridis', vmin=0, vmax=1)
        ...
        plt.imshow(true_spec_display, aspect='auto', origin='lower', cmap='viridis', vmin=0, vmax=1)
        ...
        plt.imshow(pred_spec_display, aspect='auto', origin='lower', cmap='viridis', vmin=0, vmax=1)
        ...
        normalized_true, normalized_pred = self._normalize_parameters_for_plot(param_names, true_vals, pred_vals)
        ...
        def on_click(event):
            ...
            try:
                ...
                if 'true' in title and ('waveform' in title or 'spectrogram' in title):
                    ...
                    self._play_audio_multiple_methods(results["pred_audio"], sample_idx, "predicted")
...
class InteractiveAudioEvaluator:
    ...
    def update_display(self):
        ...
        self.axes[0, 0].imshow(input_spec_display, aspect='auto', origin='lower', cmap='viridis', vmin=0, vmax=1)
        ...
        self.axes[0, 2].imshow(true_spec_display, aspect='auto', origin='lower', cmap='viridis', vmin=0, vmax=1)
        self.axes[0, 2].set_title("True Audio Spectrogram")
        
        self.axes[1, 0].imshow(pred_spec_display, aspect='auto', origin='lower', cmap='viridis', vmin=0, vmax=1)
        ...
        if param_names:
            ...
            normalized_true, normalized_pred = self.evaluator._normalize_parameters_for_plot(param_names, true_vals, pred_vals)
            
            x = np.arange(len(param_names))
            width = 0.35
            self.axes[1, 1].bar(x - width/2, normalized_true, width, label='True', alpha=0.7)
            self.axes[1, 1].bar(x + width/2, normalized_pred, width, label='Predicted', alpha=0.7)
