...
class AudioReconstructionEvaluator:
        ...
        with torch.no_grad():
            ...
            if isinstance(raw_predictions, dict):
                ...
                for head_name, logits in raw_predictions.items():
                    if hasattr(self.model, "_compute_predictions"):
                        ...
                        if head_criterion:
                            ...
                            processed_predictions[head_name] = torch.argmax(
                                logits, dim=-1
                            ).squeeze(0)
                    else:
                ...
                if self.param_names:
                    ...
                    processed_predictions = {
                        "param_0": torch.argmax(raw_predictions, dim=-1).squeeze(0)
                    }
    ...
    def _normalize_spectrogram_for_display(self, spec: np.ndarray) -> np.ndarray:
        """
        Normalize spectrogram for consistent display.

        Args:
            spec: Input spectrogram array
        ...
        if len(spec.shape) == 3:
            if spec.shape[0] == 1:  # (1, H, W) format - channels first
                spec = spec[0, :, :]  # Remove channel dimension: (1, H, W) -> (H, W)
            else:  # (H, W, 1) format - channels last
                spec = spec[:, :, 0]  # Remove channel dimension: (H, W, 1) -> (H, W)

        # Normalize for display
        if spec.dtype == np.uint8:
            return spec.astype(np.float32) / 255.0
        else:
            return np.clip(spec, 0, 1)

    def _normalize_parameters_for_plot(
        self, param_names: List[str], true_vals: List[float], pred_vals: List[float]
        ...
        Args:
            ...
            true_vals: List of true parameter values
            pred_vals: List of predicted parameter values

        Returns:
            Tuple of (normalized_true, normalized_pred) lists
        """
        normalized_true = []
        normalized_pred = []

        for i, param_name in enumerate(param_names):
            # Get parameter range from mappings if available
            if hasattr(self, "param_mappings") and param_name in self.param_mappings:
                param_min = self.param_mappings[param_name].get("min", 0.0)
                param_max = self.param_mappings[param_name].get("max", 1.0)
            else:
                # Fallback: use min/max of true and pred values
                param_min = min(true_vals[i], pred_vals[i])
                param_max = max(true_vals[i], pred_vals[i])
                if param_min == param_max:
                    param_max = param_min + 1.0

            # Normalize to [0,1]
            true_norm = (true_vals[i] - param_min) / (param_max - param_min)
            pred_norm = (pred_vals[i] - param_min) / (param_max - param_min)
            normalized_true.append(true_norm)
            normalized_pred.append(pred_norm)

        return normalized_true, normalized_pred

    def _play_audio_multiple_methods(
        self, audio: np.ndarray, sample_idx: int, audio_type: str
    ) -> None:
        """
        Play audio using multiple fallback methods.

        Args:
            audio: Audio array to play
            sample_idx: Sample index for display
            audio_type: Type of audio ("true" or "predicted")
        """
        print(f"🎵 Playing {audio_type} audio (Sample {sample_idx})")
    ...
    def plot_comparison(self, results: Dict[str, Any]) -> None:
        ...
        # Normalize input spectrogram for display
        input_spec_display = self._normalize_spectrogram_for_display(input_spec)
        ...
        plt.subplot(3, 3, 1)
        plt.imshow(
            input_spec_display, aspect="auto", origin="lower", cmap="viridis", vmin=0, vmax=1
        )
        ...
        true_spec_display = self._normalize_spectrogram_for_display(true_spec)
        plt.imshow(
            true_spec_display, aspect="auto", origin="lower", cmap="viridis", vmin=0, vmax=1
        )
        ...
        pred_spec_display = self._normalize_spectrogram_for_display(pred_spec)
        plt.imshow(
            pred_spec_display, aspect="auto", origin="lower", cmap="viridis", vmin=0, vmax=1
        )
        ...
        normalized_true, normalized_pred = self._normalize_parameters_for_plot(
            param_names, true_vals, pred_vals
        )

        x = np.arange(len(param_names))
        width = 0.35
        plt.bar(x - width / 2, normalized_true, width, label="True", alpha=0.7)
        plt.bar(x + width / 2, normalized_pred, width, label="Predicted", alpha=0.7)
        ...
        def on_click(event):
            ...
            try:
                ...
                if "true" in title and ("waveform" in title or "spectrogram" in title):
                    self._play_audio_multiple_methods(results["true_audio"], sample_idx, "true")
                elif "predicted" in title and ("waveform" in title or "spectrogram" in title):
                    self._play_audio_multiple_methods(
                        results["pred_audio"], sample_idx, "predicted"
                    )
...
class InteractiveAudioEvaluator:
    ...
    def update_display(self):
        ...
        # Normalize input spectrogram for display using shared method
        input_spec_display = self.evaluator._normalize_spectrogram_for_display(input_spec)
        ...
        # Plot spectrograms using shared normalization method
        true_params = self.current_results["true_params"]
        ...
        true_spec_display = self.evaluator._normalize_spectrogram_for_display(true_spec)
        pred_spec_display = self.evaluator._normalize_spectrogram_for_display(pred_spec)
        ...
        # Plot parameters using shared normalization method
        ...
        if param_names:
            ...
            normalized_true, normalized_pred = self.evaluator._normalize_parameters_for_plot(
                param_names, true_vals, pred_vals
            )

            x = np.arange(len(param_names))
            width = 0.35
            self.axes[1, 1].bar(x - width / 2, normalized_true, width, label="True", alpha=0.7)
            self.axes[1, 1].bar(
                x + width / 2, normalized_pred, width, label="Predicted", alpha=0.7
            )
            ...
            self.axes[1, 1].set_ylabel("Normalized Values [0-1]")
    ...
    def play_true_audio(self, event):
        """Play true audio using shared playback method."""
        ...
        try:
            ...
            self.evaluator._play_audio_multiple_methods(audio, sample_idx, "true")
        except Exception as e:
            print(f"⚠️  Error playing true audio: {e}")

    def play_pred_audio(self, event):
        """Play predicted audio using shared playback method."""
        if not self.current_results:
            print("⚠️  No audio data available")
            return
        ...
        try:
            ...
            sample_idx = self.current_results["sample_idx"]
            self.evaluator._play_audio_multiple_methods(audio, sample_idx, "predicted")
        except Exception as e:
...
def evaluate_audio_reconstruction(cfg: DictConfig) -> Dict[str, Any]:
    ...
    try:
        ...
        model: LightningModule = VIMHLitModule.load_from_checkpoint(
            ckpt_path, map_location=device, strict=False  # Allow loading with missing/extra keys
        )
        ...
        if hasattr(datamodule, "data_train") and datamodule.data_train is not None:
            ...
            if "net.embedding.pos_embedding" in checkpoint_state_dict:
                ...
                "net.conv_layers.0.weight" in checkpoint_state_dict
                or "net.features.0.weight" in checkpoint_state_dict
                or "net.conv1.weight" in checkpoint_state_dict
            ):
                ...
                if "net.conv_layers.0.weight" in checkpoint_state_dict:
                    ...
                    log.info(
                        f"Inferred CNN params: conv1_channels={conv1_channels}, conv2_channels={conv2_channels}, fc_hidden={fc_hidden}"
                    )
        ...
        for key, value in checkpoint_state_dict.items():
            if key in model_state_dict and model_state_dict[key].shape == value.shape:
                ...
                log.error(
                    f"Incompatible weight: {key} (checkpoint: {value.shape}, model: {model_state_dict.get(key, 'missing').shape if key in model_state_dict else 'missing'})"
                )
                sys.exit(1)
